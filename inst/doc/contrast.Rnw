...
<<lmTest, results = hide, echo = FALSE>>=
   ...
   if(!all.equal(lmTest$Pvalue, summary(lmFit1)$coefficients[2,4]))
...
<<glmTest, results = hide, echo = FALSE>>=
   glmTest <- contrast(
      ...
      list(diet = "high fat", group = "vehicle"),
      list(diet = "high fat", group = "treatment"))
   if(!all.equal(glmTest$Pvalue, summary(glmFit1)$coefficients[2,4]))
...
<<glsTest, results = hide, echo = FALSE>>=
   glsTest <- contrast(
         ...
         list(group = "1:A"),
         list(group = "4:D"))
   if(!all.equal(glsTest$Pvalue,  summary(glsFit)$tTable[8,4]))
...
<<lmeTest, results = hide, echo = FALSE>>=
   lmeTest <- contrast(
         ...
         list(group = "1:A"),
         list(group = "4:D"))
   if(!all.equal(lmeTest$Pvalue,  summary(lmeFit)$tTable[8,5]))
...
<<geeTest, results = hide, echo = FALSE>>=
   geeTest <- contrast(
         ...
         list(group = "1:A"),
         list(group = "4:D"))
   if(!all.equal(geeTest$Pvalue,  summary(geeFit)$mean[8,4]))
...
setwd("Z:/users/kuhna03/Code/ncstats/R/contrast/inst/doc")
options(width = 80)

Sweave(
   "Z:/users/kuhna03/Code/ncstats/R/contrast/inst/doc/contrast.Rnw",
   output = "Z:/users/kuhna03/Code/ncstats/R/contrast/inst/doc/contrast.tex")
